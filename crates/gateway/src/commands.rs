...
use gilvave_core::{dto::ws::ServerSend, security::get_access_token, settings::BASE_WS_URL};
...
use crate::{handler::handle, service::WsService};

#[tauri::command]
pub async fn listen_web_socket(state: State<'_, AppState>) -> Result<bool, ()> {
                    ...
                    while let Some(msg) = ws_reciever.next().await {
                        ...
                        match msg {
                            Ok(Message::Text(text)) => {
                                ...
                                handle(state.sender.clone(), text.to_string()).await;
                            }
                            Ok(Message::Close(_)) => {
                                tracing::warn!("WebSocket connection closed");
                                // *web_socket.lock().await = None;
                                break;
                            }
                            Err(e) => {
...
#[tauri::command]
pub async fn join_channel(state: State<'_, AppState>) -> Result<(), ()> {
    tracing::info!("join_channel 0");
    WsService::join_channel(state.sender.clone()).await;
    tracing::info!("join_channel 999");
    Ok(())
}
